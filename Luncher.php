<?php
define('BOT_TOKEN', '233056544:AAH8dF3iposk1mk5drnc2CzguTg0pGbcU0s');
...
$substr = substr($text, 0, 28);
	if (!in_array($chat_id,$اعضاid)) {
		apiRequest("forwardMessage", array('chat_id' => $admin,  "from_chat_id"=> $chat_id ,"message_id" => $message_id));
apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => $pاعضاiddd[1] ,"parse_mode" =>"HTML"));	
}else{
...
لطفا پیام ندهید🖕" ,"parse_mode" =>"HTML"));	
    ...
    $caption = $message['caption'];
		...
		$ttxtt = file_get_contents('pاعضا.txt');
		$اعضاidd= explode("\n",$ttxtt);
		for($y=0;$y<count($اعضاidd);$y++){
			if($caption != "")
    {
    apiRequest("sendphoto", array('chat_id' => $اعضاidd[$y], "photo" => $substtr,"caption" =>$caption));
    }
    else{
        apiRequest("sendphoto", array('chat_id' => $اعضاidd[$y], "photo" => $substtr));
    }
			
		}
		$memcout = count($اعضاidd)-1;
	 	apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "📦 پیام شما به  ".$memcout." مخاطب ازسال شد.
.","parse_mode" =>"HTML",'reply_markup' => array(
        'keyboard' => array(array('🗣 پیام همگانی'),array('  راهنما','  اعضا','  کاربران مسدود شده'),array("تنطیمات  ")),
   ...
   $video = $message['video']['file_id'];
    ...
    $caption = $message['caption'];
		...
		for($y=0;$y<count($اعضاidd);$y++){
			if($caption != "")
    {
    apiRequest("sendvideo", array('chat_id' => $اعضاidd[$y], "video" => $video,"caption" =>$caption));
    }
    else{
        apiRequest("sendvideo", array('chat_id' => $اعضاidd[$y], "video" => $video));
    }
		}
		$memcout = count($اعضاidd)-1;
	 	apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "📦 پیام شما به  ".$memcout." مخاطب ازسال شد.
.","parse_mode" =>"HTML",'reply_markup' => array(
        'keyboard' => array(array('🗣 پیام همگانی'),array('⚓️ راهنما','👥 اعضا','❌ کاربران مسدود شده'),array("منوی تنطیمات ⚙")),
        'one_time_keyboard' => true,
        'selective' => true,
        'resize_keyboard' => true))); 
         $addd = "false";
   ...
   $sticker = $message['sticker']['file_id'];
	...
	apiRequest("sendMessage", array('chat_id' => $chat_id, "text" =>"🗣پیام شما ارسال شد. " ,"parse_mode" =>"HTML"));
       ...
       $sticker = $message['sticker']['file_id'];
		...
		$اعضاidd= explode("\n",$ttxtt);
		for($y=0;$y<count($اعضاidd);$y++){
			//apiRequest("sendMessage", array('chat_id' => $اعضاidd[$y], "text" => $texttoall,"parse_mode" =>"HTML"));
			
			    apiRequest("sendsticker", array('chat_id' => $اعضاidd[$y], "sticker" => $sticker));
...
لطفا پیام ندهید🖕" ,"parse_mode" =>"HTML"));	
   ...
   $video = $message['document']['file_id'];
    ...
    apiRequest("sendDocument", array('chat_id' => $اعضاidd[$y], "document" => $video));
...
لطفا پیام ندهید🖕" ,"parse_mode" =>"HTML"));	
   ...
   $video = $message['voice']['file_id'];
    ...
    $video = $message['voice']['file_id'];
        ...
        apiRequest("sendVoice", array('chat_id' => $اعضاidd[$y], "voice" => $video));
		}
		$memcout = count($اعضاidd)-1;
	 	apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "📦 پیام شما به  ".$memcout." مخاطب ازسال شد.
.","parse_mode" =>"HTML",'reply_markup' => array(
   ...
   $video = $message['audio']['file_id'];
    ...
    $video = $message['audio']['file_id'];
		...
		for($y=0;$y<count($اعضاidd);$y++){
                ...
                apiRequest("sendaudio", array('chat_id' => $اعضاidd[$y], "audio" => $video));
...
لطفا پیام ندهید🖕" ,"parse_mode" =>"HTML"));	
   ...
   $phone = $message['contact']['phone_number'];
    ...
    apiRequest("sendcontact", array('chat_id' => $اعضاidd[$y], "phone_number" => $phone,"Last_name" =>$last,"first_name"=> $first));
...
.","parse_mode" =>"HTML",'reply_markup' => array(
  ...
  if (isset($message['text'])) {
    ...
    if ($text=="/start") {
      ...
      if($chat_id!=$admin){
      apiRequest("sendMessage", array('chat_id' => $chat_id,"text"=>$pاعضاiddd[0] ,"parse_mode"=>"HTML"));

$txxt = file_get_contents('pاعضا.txt');
$pاعضاid= explode("\n",$txxt);
	if (!in_array($chat_id,$pاعضاid)) {
		$aaddd = file_get_contents('pاعضا.txt');
		$aaddd .= $chat_id."
";
    	file_put_contents('pاعضا.txt',$aaddd);
}
...
."));
    ...
    file_put_contents('msgs.txt',$pاعضاiddd[0]."
...
apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => $pاعضاiddd[1] ,"parse_mode" =>"HTML"));	
    ...
    }else if ($text == "منوی تنطیمات ⚙" && $chat_id==$admin) {
...
در صورت انصراف بازگشت را بزنید
.', 'reply_markup' => array(
        'keyboard' => array(array('❌ حدف اعضا  ','❌ حدف   لیست سیاه '),array('🔙 بازگشت')),
        'one_time_keyboard' => true,
        'selective' => true,
        'resize_keyboard' => true)));
...
.","parse_mode" =>"MARKDOWN",'reply_markup' => array(
    ...
    }else if ($text == "❌ حدف اعضا اعضا" && $chat_id==$admin) {
    		
    		
    		$txxt = file_get_contents('pاعضا.txt');
        $pاعضاid= explode("\n",$txxt);
    		file_put_contents('pاعضا.txt',"");
...
.', 'reply_markup' => array(
    ...
    else if ($text == "❌ حدف اعضا  لیست سیاه" && $chat_id==$admin) {
...
.","parse_mode" =>"HTML",'reply_markup' => array(
		...
		$txtt = file_get_contents('pاعضا.txt');
		$اعضاidd= explode("\n",$txtt);
		$mmemcount = count($اعضاidd) -1;
	...
	}else if($text == "❌ کاربران مسدود شده" && $chat_id == $admin ){
